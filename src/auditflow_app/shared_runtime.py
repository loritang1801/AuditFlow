...
def load_shared_agent_platform():
    preferred_source = (os.getenv("AUDITFLOW_SHARED_CORE_SOURCE") or "vendored").strip().lower()
    if preferred_source not in {"vendored", "workspace"}:
        raise ValueError("AUDITFLOW_SHARED_CORE_SOURCE must be 'vendored' or 'workspace'")
